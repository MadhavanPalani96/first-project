...
/*AbstractApplicationContext-Sometimes it is necessary to manage the initialize and destroy of a spring bean, 
when a spring bean is being initialized or destroyed. There are set of interfaces
provided by spring framework that defines methods to manage initialization and destroy of a spring bean.*/

/*AnnotationConfigApplicationContext-Bean in spring can be created in java file instead of XML,too. They are created with the help of @Configuration. 
And that application configuration is loaded by AnnotationConfigApplicationContext in spring container. 
AnnotationConfigApplicationContext has the register method that accepts the bean configuration class.*/
...
Callers may retain references to returned objects in the case of Singleton beans.*/
		...
		case 3:
			...
			System.out.println("Please Enter the Category ID  for updation");
			int CategoryId5 = sc.nextInt();
			Category category7 = categoryDAO.findCategoryById(CategoryId5);
			System.out.println("Please Enter the Category details for updation");
			
			System.out.println("CategoryName:");

			category7.setCategoryName(sc.next());
			
	
			
			boolean result2 = categoryDAO.updateCategory(category7);
			if(result2 == true)
			{
